  ...
  try {
    if (mode === "create-account") {
      ...
      if (trimmedName.length < 2) {
        return parseStringify({
          sessionId: null,
          error: "Full name is required to finish account creation.",
        }) as VerifyOTPResult;
      }
      ...
      if (!normalizedEmail) {
        return parseStringify({
          sessionId: null,
          error: "Email is required to finish account creation.",
        }) as VerifyOTPResult;
      }
    }
    ...
    if (mode === "create-account") {
      const trimmedName = fullName!.trim();
      const emailFromAuth = normalizeEmail(email!);
      ...
      if (existingByAccount.total === 0) {
        ...
        if (existingByEmail) {
          return parseStringify({ sessionId: session.$id, error: null }) as VerifyOTPResult;
        }
