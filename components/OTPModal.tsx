...
const OTPModal = ({
  ...
  // Keep verification account id in sync with latest OTP send.
  ...
  const handleSubmit = async (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    try {
      ...
      if (result?.sessionId) {
        ...
        setSubmitError(
          result?.error ||
            "Invalid or expired OTP. Please try again or resend a new code."
        );
      }
    } catch (error) {
  ...
  const handleResendOtp = async () => {
    ...
    try {
      ...
      if (otp?.accountId) {
        setActiveAccountId(otp.accountId);
      } else {
        setSubmitError(otp?.error || "Failed to resend OTP. Please try again.");
      }
    } catch {
  ...
  return (
    <AlertDialog open={isOpen} onOpenChange={setIsOpen}>
      <AlertDialogContent className="shad-alert-dialog">
        ...
        {submitError && <p className="error-message text-center">*{submitError}</p>}
